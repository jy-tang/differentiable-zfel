...
def params_from_K_profile(K_profile, base_params, base_input):
    ...
    # Deterministic reference case uses no injected seed.
    E02 = jnp.asarray(0.0)
...
def main():
    ...
    n_ctrl = 8
    W = build_interp_matrix(n_steps, n_ctrl)
    ...
    def objective(theta_local, lambda_smooth=5e2, lambda_mono=5e3):
        ...
        return jnp.log(pulse) - lambda_smooth * smooth_pen - lambda_mono * mono_pen, (
            pulse,
            K_profile,
            smooth_pen,
            mono_pen,
    ...
    base_out = sase1d.sase_from_initial_conditions_jax(
        params_from_K_profile(K0, base_params, base_input), bucket
    )
    ref_out = sase1d.sase_from_initial_conditions_jax(
        params_from_K_profile(K_ref, base_params, base_input), bucket
    )

    base_pulse = float(pulse_energy_like(base_out))
    ref_pulse = float(pulse_energy_like(ref_out))

    print("Deterministic reference setup")
    print(f"  no-taper pulse-energy-like objective: {base_pulse:.6e}")
    print(f"  no-taper final power [GW]:            {float(base_out['power_z'][-1]) / 1e9:.3f}")
    print(f"  reference taper pulse-energy-like:    {ref_pulse:.6e}")
    print(f"  reference taper final power [GW]:     {float(ref_out['power_z'][-1]) / 1e9:.3f}")
    print(f"  reference taper ratio:                {ref_pulse / base_pulse:.3f}x")
    print()
    ...
    for t in range(1, n_iter + 1):
        (obj, aux), g = jax.value_and_grad(objective, has_aux=True)(theta)
        pulse, K_profile, smooth_pen, mono_pen = aux
        ...
        ratio = pulse_val / base_pulse
        if best is None or pulse_val > best["pulse"]:
            best = {
                "iter": t,
                "pulse": pulse_val,
                "ratio": ratio,
                ...
                "mono_pen": float(mono_pen),
            }

        if t in [1, 2, 5, 10]:
            print(
                f"iter={t:02d} obj={float(obj):.6f} pulse={pulse_val:.6e} "
                f"ratio={ratio:.3f}x smooth_pen={float(smooth_pen):.3e} mono_pen={float(mono_pen):.3e}"
            )

    best_params = params_from_K_profile(jnp.asarray(best["K_profile"]), base_params, base_input)
    best_out = sase1d.sase_from_initial_conditions_jax(best_params, bucket)

    print()
    print("Best gradient-optimized taper")
    print(f"  best iteration:                 {best['iter']}")
    print(f"  pulse-energy-like objective:    {best['pulse']:.6e}")
    print(f"  final power [GW]:               {float(best_out['power_z'][-1]) / 1e9:.3f}")
    print(f"  pulse-energy ratio vs no taper: {best['ratio']:.3f}x")
