...
OUTDIR = Path("docs/examples/generated_plots/seeded_gradient_piecewise_linear_p2e8_2026-04-15")
N_KNOTS = 6
...
def render_k_profile(theta, z_grid, k0):
    start_frac = 0.00 + 0.30 * jax.nn.sigmoid(theta[0])
    knot_increments = DELTA_SCALE * (jax.nn.softplus(theta[1:]) - LOG2)
...
def warm_start_from_reference():
    # Best 2e8 quadratic run from seeded_gradient_restart_p2e8_2026-04-15.
    theta_ref = jnp.asarray(
        [-1.1240125894017352, 0.06436973778486459, 0.08317621151472483],
        dtype=jnp.float64,
    )
    z_grid = jnp.linspace(0.0, 1.0, BASE_INPUT["z_steps"])
    k_ref, start_frac = render_reference_quadratic(theta_ref, z_grid, K0)
    taper_ref = jnp.maximum(1.0 - k_ref / K0, 0.0)
    tail_span = max(1.0 - float(start_frac), 1e-6)
    u_knots = np.asarray(KNOT_X)
    taper_knots_ref = np.interp(
        u_knots,
        np.asarray(jnp.clip((z_grid - start_frac) / tail_span, 0.0, 1.0)),
        np.asarray(taper_ref),
    )
    ...
    theta0 = np.concatenate(
        [
            [inv_sigmoid(float(start_frac) / 0.30)],
            [inv_centered_softplus(max(v, 1e-6), DELTA_SCALE) for v in knot_increments_ref],
        ]
    )
...
def main():
    ...
    print("Seeded FEL piecewise-linear gradient taper")
    ...
    best_start_frac = float(0.00 + 0.30 / (1.0 + math.exp(-best["theta"][0])))
    summary = {
        "run_date": "2026-04-15",
        ...
        "best_start_frac": best_start_frac,
        "n_piecewise_knots": N_KNOTS,
    }

    (OUTDIR / "seeded_gradient_piecewise_linear_summary.json").write_text(
    ...
    ax.plot(z_grid, best["k_profile"], label="Piecewise-linear taper", linewidth=2.6, color="#0E7490")
    ax.set_xlabel("Undulator position z (m)")
    ax.set_ylabel("Undulator parameter K")
    ax.set_title("Seeded FEL piecewise-linear taper profile")
    ax.legend(frameon=True)
    fig.tight_layout()
    fig.savefig(OUTDIR / "seeded_gradient_piecewise_linear_taper_profile.png", dpi=180)
    plt.close(fig)
    ...
    ax.plot(z_grid, best["power_z_gw"], label="Piecewise-linear taper", linewidth=2.6, color="#0E7490")
    ax.set_yscale("log")
    ax.set_xlabel("Undulator position z (m)")
    ax.set_ylabel("Power (GW)")
    ax.set_title("Seeded FEL power growth with piecewise-linear taper")
    ax.legend(frameon=True)
    fig.tight_layout()
    fig.savefig(OUTDIR / "seeded_gradient_piecewise_linear_power_growth.png", dpi=180)
    plt.close(fig)
    ...
    axes[0].set_title("Power gain during piecewise-linear optimization")
    ...
    axes[1].set_title("Pulse-energy gain during piecewise-linear optimization")
    fig.tight_layout()
    fig.savefig(OUTDIR / "seeded_gradient_piecewise_linear_history.png", dpi=180)
    plt.close(fig)

    print()
    print("Best seeded piecewise-linear taper")
    ...
    print(f"  start fraction:                {best_start_frac:.4f}")
    print(f"  min(K):                        {float(np.min(best['k_profile'])):.3f}")
    print(f"  max(K):                        {float(np.max(best['k_profile'])):.3f}")
    print()
    print("Saved artifacts:")
    print(f"  {OUTDIR / 'seeded_gradient_piecewise_linear_taper_profile.png'}")
    print(f"  {OUTDIR / 'seeded_gradient_piecewise_linear_power_growth.png'}")
    print(f"  {OUTDIR / 'seeded_gradient_piecewise_linear_history.png'}")
    print(f"  {OUTDIR / 'seeded_gradient_piecewise_linear_summary.json'}")
