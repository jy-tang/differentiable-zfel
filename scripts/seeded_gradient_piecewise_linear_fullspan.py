...
OUTDIR = Path("docs/examples/generated_plots/seeded_gradient_piecewise_linear_fullspan_fix_p2e8_2026-04-15")
N_KNOTS = 12
...
def warm_start_from_reference():
    # Warm-start from the best 7-parameter piecewise-linear run, then force
    # the new parameterization to start tapering at z=0.
    z_grid = np.linspace(0.0, 1.0, BASE_INPUT["z_steps"])
    piecewise_summary = json.loads(
        Path(
            "docs/examples/generated_plots/seeded_gradient_piecewise_linear_p2e8_2026-04-15/"
            "seeded_gradient_piecewise_linear_summary.json"
